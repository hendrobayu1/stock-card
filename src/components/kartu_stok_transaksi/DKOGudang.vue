<template>
    <v-card>
            ...
            <v-list-item>
                ...
                <v-row>
                    <v-col cols="12" md="5" sm="5">
                        ...
                        type="text" clear-icon="mdi-close-circle" clearable :disabled="cekExistingObjDKO()" @keydown.enter="onButtonClick(null,2)">
                                ...
                                <v-dialog v-model="dialog_kode_transaksi" persistent max-width="1000px">
                                    ...
                                    <v-card>
                                        <v-card-title class="headline grey lighten-2">
                                            <span class="text-sm-body-1 font-weight-bold">Transaksi DKO Gudang ({{pengguna.unit}})</span>
                                        </v-card-title>
                    ...
                    <v-col cols="4" md="3" sm="3">
                        <v-text-field class="text-sm-body-2" v-model="nomor_dko" label="Nomor DKO" 
                        type="text" clear-icon="mdi-close-circle" clearable :disabled="cekExistingObjDKO()" />
                    </v-col>
                    <v-col cols="8" md="4" sm="4">
                        <v-menu v-model="date_menu" :close-on-content-click="false" :nudge-right="5" 
                        transition="scale-transition" offset-y max-width="290px">
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field class="text-sm-body-2" v-model="computedDateFormat" label="Tanggal DKO"
                                readonly v-bind="attrs" v-on="on">
                                    <template v-slot:append-outer>
                                        <v-btn class="mx-1" dark color="indigo" :disabled="cekExistingObjDKO()" @click="onButtonClick(null,2)">
                                            <v-icon dark>mdi-magnify</v-icon>
                                        </v-btn>
                                    </template>
                                </v-text-field>
                            </template>
                            <v-date-picker v-model="date_dko" @input="date_menu = false" :disabled="cekExistingObjDKO()"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
                <!-- </v-list-item-title> -->
        ...
        <v-list three-line subheader>
            <v-subheader>Informasi DKO</v-subheader>
            <v-list-item>
                <v-row>
                    <v-col cols="3" md="3" sm="3">
                        <v-text-field class="text-sm-body-2" v-model="objInfoDKO.nomor" label="No.DKO" readonly outlined dense placeholder="" />
                    </v-col>
                    <v-col cols="3" md="3" sm="3">
                        <v-text-field class="text-sm-body-2" v-model="computedDateFormat_DKOTrans" label="Tgl DKO" readonly outlined dense placeholder="" />
                    </v-col>
                    <v-col cols="6" md="6" sm="6">
                        <v-text-field class="text-sm-body-2" v-model="objInfoDKO.user_proses" label="Petugas" readonly outlined dense placeholder="" />
                    </v-col>
                </v-row>
            </v-list-item>
        </v-list>
            ...
            <v-list-item>
                <v-expansion-panels inset focusable multiple v-model="panel">
                    <v-expansion-panel :key="0">
                        <v-expansion-panel-header><strong>DKO GUDANG</strong></v-expansion-panel-header>
                        <v-expansion-panel-content>
                            <!-- v-model="selected" :single-select="singleSelect" show-select -->
                            <v-data-table dense :headers="headers_nota_DKO" :items="nota_DKO" 
...
export default {
    ...
    data:()=>({
        ...
        headers_kode_transaksi:[
            ...
            {text: 'Tgl DKO',align: 'start',sortable: false,value: 'tgl',},
        ...
        nomor_dko:'',
        date_menu:false,
        date_dko:new Date().toISOString().substr(0, 10),
        objInfoDKO:{},
        panel:[0],
    ...
    computed:{
        ...
        computedDateFormat(){
            return this.formatDateTime(this.date_dko,8)
        },
        ...
        computedDateFormat_DKOTrans(){
            if(typeof this.objInfoDKO.tgl != "undefined"){
                return this.formatDateTime(this.objInfoDKO.tgl,8)
            }else{
                return this.objInfoDKO.tgl
            }
        },
    },
    methods:{
        ...
        listRakBarang(){
            let kode = []
            for(var i=0;i<this.nota_DKO.length;i++){
                kode.push(this.nota_DKO[i]['kode_barang_phc'])
            }
        ...
        cekExistingObjDKO(){
            return Object.prototype.hasOwnProperty.call(this.objInfoDKO, "id_transaksi")
        },
        cancelProses(){
            if(typeof this.objInfoDKO.id_transaksi != "undefined"){

                let fr=new FormData()
                fr.set('kode_transaksi',this.objInfoDKO.id_transaksi)
                ...
                // console.log(this.objInfoDKO.id_transaksi)
        ...
        onButtonClick(item,idxCari) {
            ...
            if(idxCari==1){
                ...
                fr.set('nomor_dko',null)
                fr.set('tgl_dko',null)
             }else if(idxCari==2){
                fr.set('kode_transaksi',this.kode_transaksi)
                fr.set('nomor_dko',this.nomor_dko)
                fr.set('tgl_dko',this.date_dko)
            }
            let config = {
                headers:{
                    'Authorization' : 'Bearer ' + this.pengguna.api_token,
            ...
            // console.log(config)
            this.objInfoDKO = {}
            if((this.nomor_dko=='' || this.nomor_dko=='0') && this.kode_transaksi=='' && idxCari==2){
                this.setNotif({
                    status:true,
                    color:'error',
                ...
                this.axios.post('kartu-stok/cari-transaksi-dko',fr,config)
                .then((response)=>{
                    // console.log(response)
                    if(response.data.status=='success'){
                        console.log(response.data.data)
                        this.kode_transaksi = response.data.data.id_transaksi
                        this.objInfoDKO = response.data.data
                        this.nota_DKO = response.data.data.rincian
                        this.dialog_kode_transaksi = false
                        // console.log(this.nota_DKO)
                        this.listRakBarang()
                        this.setNotif({
                            status:true,
                            color:'success',
        ...
        getListDKO(){
            ...
            this.axios.post('kartu-stok/list-transaksi-dko',fr,config)
            .then((response)=>{
                // console.log(response)
        ...
        saveProses(){
            if(typeof this.objInfoDKO == 'undefined' || this.nota_DKO.length<1){
                this.setNotif({
                    status:true,
                    color:'error',
                ...
                let frdata =    {
                    ...
                    'id_transaksi' : this.objInfoDKO.id_transaksi,
                    'tgl_transaksi_dko' : this.objInfoDKO.tgl,
                    'jenis_transaksi' : this.objJenisTransaksi.kode,
                    'mutasi' : this.objJenisTransaksi.mutasi,
                    'transaksi' : JSON.stringify(this.nota_DKO),
                ...
                // console.log(this.objInfoDKO.id_transaksi)
                // console.log(this.objInfoDKO.tgl)
                // console.log(this.objJenisTransaksi.kode)
                // console.log(this.objJenisTransaksi.mutasi)
                // console.log(JSON.stringify(this.nota_DKO))
                // console.log(config)
                this.axios.post('/kartu-stok/simpan-transaksi-dko',frdata,config)
                .then((response)=>{
                    // console.log(response)
